# flex

This is the frontend for **Flex**, a responsive fitness workout browser that lets users search exercises, view details, and "like" workouts to save them for later. Built with **React** and **Vite**, and designed to work with mocked backend responses for seamless testing and development.

> Deployed on:

---

## Features

- **Search Workouts**: Filter exercises by name or category.
- **Like / Save Workouts**: Click the heart to add a workout to your profile.
- **Authentication Simulation**: Log in and register with mock API (stored in `localStorage`).
- **Profile Page**: View your saved workouts.

---

## Acknowledgement

WGER API— for exercise data.
